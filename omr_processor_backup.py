...
def _read_answers(binary, y_rows, profile):
    ...
    snap      = profile.get('snap_range', 4)  # búsqueda ±N px alrededor
    edge_margin = radius + 6  # margen de seguridad contra bordes de imagen
    ...
    for col in profile['columns']:
        ...
        for row_idx in range(n_rows):
            ...
            for x in xs:
                best_fill = 0.0
                # Bubble snapping: probar posiciones vecinas y quedarse
                # con la de mayor fill. Compensa desalineación entre la
                # posición teórica y la burbuja real.
                for dy in range(-snap, snap + 1, 2):
                    for dx in range(-snap, snap + 1, 2):
                        cy = y + dy
                        cx = x + dx
                        # No desplazar hacia los bordes de la imagen:
                        # evita capturar artefactos del borde/perspectiva
                        if cx - radius < edge_margin or cx + radius > w - edge_margin:
                            continue
                        if cy - radius < edge_margin or cy + radius > h - edge_margin:
                            continue
                        y1 = cy - radius
                        y2 = cy + radius
                        x1 = cx - radius
                        ...
                        f = float(np.sum(roi > 127)) / roi.size if roi.size else 0.0
                        if f > best_fill:
                            best_fill = f
...
def _pick_answer(fills, options, threshold, min_contrast):
    ...
    # ── Criterio 1: threshold absoluto ───────────────────────────────────────
    abs_pass = mx >= threshold

    # ── Criterio 2: row-relative ─────────────────────────────────────────────
    # Compara la burbuja más llena contra el promedio de las demás.
    # Si tiene ≥2× el promedio de las otras, es una marca clara aunque
    # el fill absoluto sea bajo (marca suave, ROI parcial, etc.)
    others    = [f for j, f in enumerate(fills) if j != mi]
    mean_rest = sum(others) / len(others) if others else 0.0
    rel_pass  = mx > 0.003 and (mean_rest < 0.001 or mx / (mean_rest + 1e-6) >= 2.0)

    # ── Criterio 3: candidato dominante ──────────────────────────────────────
    # Si una burbuja concentra >50% del fill total de la fila y es ≥1.5×
    # la segunda, es la mejor candidata aunque el ROI no cubra toda la
    # burbuja real (cobertura parcial por desalineación).
    total_fill = sum(fills)
    sf         = sorted(fills, reverse=True)
    dominant   = (total_fill > 0.003
                  and mx / (total_fill + 1e-6) > 0.50
                  and sf[0] >= sf[1] * 1.5)

    if not abs_pass and not rel_pass and not dominant:
        return '?', 0.0

    contrast = sf[0] - sf[1]

    # Contraste reducido para criterios secundarios
    if abs_pass:
        effective_contrast = min_contrast
    elif rel_pass:
        effective_contrast = min_contrast * 0.4
    else:
        effective_contrast = min_contrast * 0.3

    if contrast < effective_contrast:
        return '?', 5.0
